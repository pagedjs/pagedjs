...
describe('page-spread', async () => {
    ...
    it('should render page 2 as left', async () => {
			let isLeft = await page.$eval("#page-2", (r) => {
    ...
    it('should render page 3 as right', async () => {
      let isRight = await page.$eval("#page-3", (r) => {
      ...
      expect(isRight).toEqual(true);
		...
		it('should have bottom center text of right on the first page', async () => {
			let text = await page.$eval("#page-1 .pagedjs_margin-bottom-center > .pagedjs_margin-content", (r) => window.getComputedStyle(r, '::after').content);
			expect(text).toContain("right");
		})

    it('should have a yellow background on page 6', async () => {
			let textColor = await page.$eval("#page-6", (r) => window.getComputedStyle(r).backgroundColor);
